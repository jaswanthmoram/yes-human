...
steps:
  - Confirm the requested ssl/tls configuration review outcome, scope, owner, and success criteria
...
Use this skill when a task explicitly matches `security.ssl-tls-config` or when the user asks for ssl/tls configuration review support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
**Example A:** A user asks for ssl/tls configuration review help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
