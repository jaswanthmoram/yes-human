...
steps:
  - Confirm the requested jwt token validation outcome, scope, owner, and success criteria
...
## Trigger
Use this skill when a task explicitly matches `security.jwt-validation` or when the user asks for jwt token validation support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
## Examples
**Example A:** A user asks for jwt token validation help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
