...
steps:
  - Confirm the requested rate limiting implementation outcome, scope, owner, and success criteria
...
Use this skill when a task explicitly matches `security.rate-limiting` or when the user asks for rate limiting implementation support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
**Example A:** A user asks for rate limiting implementation help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
