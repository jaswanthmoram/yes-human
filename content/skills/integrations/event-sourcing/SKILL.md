...
steps:
  - Confirm the requested event sourcing outcome, scope, owner, and success criteria
...
Use this skill when a task explicitly matches `integrations.event-sourcing` or when the user asks for event sourcing support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
**Example A:** A user asks for event sourcing help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
