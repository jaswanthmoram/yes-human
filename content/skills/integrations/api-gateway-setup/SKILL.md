...
triggers:
  ...
  - yes human task
  - api gateway setup review
  - api gateway setup checklist
  - api gateway setup plan
activation_triggers:
  - deploy api gateway
  - gateway infrastructure
  - help me with api gateway setup
prerequisites:
...
steps:
  - Confirm the requested api gateway setup outcome, scope, owner, and success criteria
...
Use this skill when a task explicitly matches `integrations.api-gateway-setup` or when the user asks for api gateway setup support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
**Example A:** A user asks for api gateway setup help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
