...
steps:
  - Confirm the requested rate limiting outcome, scope, owner, and success criteria
...
## Trigger
Use this skill when a task explicitly matches `integrations.rate-limiting` or when the user asks for rate limiting support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
## Examples
**Example A:** A user asks for rate limiting help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
