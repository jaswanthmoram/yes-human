...
triggers:
  ...
  - yes human task
  - code generation for registry artifacts review
activation_triggers:
  - code generation
...
steps:
  - Confirm the requested code generation for registry artifacts outcome, scope, owner, and success criteria
...
Use this skill when a task explicitly matches `meta-system.code-generation` or when the user asks for code generation for registry artifacts support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
**Example A:** A user asks for code generation for registry artifacts help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
