...
triggers:
  ...
  - CI pipeline review
  - yes human task
  - continuous integration for registry validation review
activation_triggers:
  - CI setup
  - pipeline design
...
steps:
  - Confirm the requested continuous integration for registry validation outcome, scope, owner, and success criteria
...
## Trigger
Use this skill when a task explicitly matches `meta-system.continuous-integration` or when the user asks for continuous integration for registry validation support. It is designed for bounded task work where the agent needs concrete inputs, a repeatable procedure, and verification before handoff.
...
## Examples
**Example A:** A user asks for continuous integration for registry validation help with a specific file or dataset; apply the six-step procedure and return a concise, evidence-backed artifact.
