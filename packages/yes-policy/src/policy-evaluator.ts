...
import { COMBINED_SECRET_REGEX } from "@yes-human/core";
 ...
 */
const _evaluatorCache = new Map<string, PolicyEvaluator>();

export function getSharedPolicyEvaluator(config: any = {}): PolicyEvaluator {
...
export class PolicyEvaluator {
  ...
  constructor(config: any = {}) {
    ...
    this.costPolicyPath =
      (isString ? config : config.costPolicyPath) ||
  ...
  loadRules(): any {
    ...
    for (const file of files) {
      try {
        ...
        console.error(
          `[PolicyEvaluator] Failed to load rule ${file}:`,
          error.message
        );
      }
    }
  ...
  loadPolicies(): any {
    ...
    if (!fs.existsSync(policiesPath)) {
      console.warn(
        `[PolicyEvaluator] Policies directory not found: ${policiesPath}`
      );
      return policies;
    }
    ...
    for (const file of files) {
      try {
        ...
        console.error(
          `[PolicyEvaluator] Failed to load policy ${file}:`,
          error.message
        );
      }
    }
  ...
  loadContracts(): any {
    ...
    for (const file of files) {
      try {
        ...
        console.error(
          `[PolicyEvaluator] Failed to load contract ${file}:`,
          error.message
        );
      }
    }
  ...
  loadCostPolicy(): any {
    try {
      const content = fs.readFileSync(
        path.join(process.cwd(), this.costPolicyPath),
        "utf8"
      );
   ...
   * @param {object} rawContext - The context to evaluate.
   * @returns {{ allowed: boolean, reason?: string, rule?: string, policy?: string }} Evaluation result.
   */
  evaluate(rawContext: any): any {
    const context = { ...rawContext };
    if (context.filePath !== undefined && context.path === undefined)
      context.path = context.filePath;
    if (context.path !== undefined && context.filePath === undefined)
    ...
    if (!this._rulesByAction) {
      ...
      for (const [ruleId, rule] of Object.entries(this.rules)) {
        for (const act of (rule as any).applies_to || []) {
    ...
    if (!this._policiesByAction) {
      ...
      for (const [policyId, policy] of Object.entries(this.policies)) {
        const applies = (policy as any).applies_to || [];
   ...
   */
  matchCondition(condition: any, context: any): boolean {
   ...
   */
  matchOne(value: any, operator: any, context: any): boolean {
    ...
    const known = (k: string) =>
      operator[k] !== undefined && (sawKnownOperator = true);

    if (known("$eq") && value !== operator.$eq) return false;
    if (known("$ne") && value === operator.$ne) return false;

    if (known("$gt") && !(typeof value === "number" && value > operator.$gt))
      return false;
    if (known("$gte") && !(typeof value === "number" && value >= operator.$gte))
      return false;
    if (known("$lt") && !(typeof value === "number" && value < operator.$lt))
      return false;
    if (known("$lte") && !(typeof value === "number" && value <= operator.$lte))
      return false;
    ...
    if (known("$contains")) {
      ...
      const terms = Array.isArray(operator.$contains)
        ? operator.$contains
        : [operator.$contains];
      if (
        !terms.some((term) =>
          searchValue.toLowerCase().includes(String(term).toLowerCase())
        )
      ) {
        return false;
      }
    ...
    if (known("$matches")) {
      ...
      try {
        if (!new RegExp(operator.$matches, "i").test(String(value)))
          return false;
      } catch {
        return false;
      }
    ...
    if (known("$not_matches")) {
      try {
        if (
          value !== undefined &&
          new RegExp(operator.$not_matches, "i").test(String(value))
        )
          return false;
      } catch {
        return false;
    ...
    if (known("$starts_with")) {
      if (
        value === undefined ||
        !String(value).startsWith(operator.$starts_with)
      )
        return false;
    }
    ...
    if (known("$contains_current_route")) {
      ...
      const present =
        ...
        value.includes(context.current_route);
  ...
  stringifyForSearch(value: any): string {
   ...
   */
  checkContract(contract: any, context: any): any {
   ...
   */
  evaluatePrecondition(precondition: string, context: any): boolean {
    if (typeof precondition !== "string" || !precondition.includes(":")) {
      console.warn(
        `[PolicyEvaluator] Malformed precondition (missing prefix): ${JSON.stringify(
          precondition
        )}`
      );
  ...
  /**
   * Evaluate an invariant string.
   */
  evaluateInvariant(invariant: string, context: any): boolean {
    if (typeof invariant !== "string") {
      console.warn(
        `[PolicyEvaluator] Malformed invariant (not a string): ${JSON.stringify(
          invariant
        )}`
      );
      return process.env.YES_LENIENT_DSL === "true";
    ...
    if (invariant.startsWith("max_tokens:")) {
      ...
      if (Number.isNaN(maxTokens)) {
        console.warn(
          `[PolicyEvaluator] max_tokens invariant has non-numeric value: ${invariant}`
        );
        return process.env.YES_LENIENT_DSL === "true";
      }
    ...
    const prefix = invariant.includes(":")
      ? invariant.split(":")[0]
      : invariant;
    console.warn(
  ...
  /**
   * Check if a request is within token budget
   */
  isWithinBudget(band: string, estimatedTokens: number): boolean {
    const limits =
      this.costPolicy.bands[band] ||
      this.costPolicy.costPolicy.bands[
        this.costPolicy.costPolicy.default_budget_band
      ];
