...
export class StateMachine {
  ...
  stateByName: Map<string, any>;
  current: string;

  constructor(definition: any) {
    this.def = definition;
    this.stateNames = new Set((definition.states || []).map((s: any) => s.name));
    this.stateByName = new Map((definition.states || []).map((s: any) => [s.name, s]));
    this.current = definition.initial;
  }

  static fromFile(filePath: string): StateMachine {
    return new StateMachine(JSON.parse(fs.readFileSync(filePath, "utf8")));
  }

  /** Events available from a given state. */
  eventsFrom(state: string): string[] {
    return (this.def.transitions || [])
      .filter((t: any) => t.from === state)
      .map((t: any) => t.on);
  }

  /** Resolve the target state for (from, event), or null if no such transition. */
  target(from: string, event: string): string | null {
    const t = (this.def.transitions || []).find(
      (x: any) => x.from === from && x.on === event
    );
    return t ? t.to : null;
  }
  ...
  send(event: string): string {
    ...
    if (to === null) {
      throw new Error(
        `Invalid transition: '${event}' from state '${this.current}' in ${this.def.id}`
      );
    }
    this.current = to;
    return to;
  ...
  isTerminal(state: string = this.current): boolean {
  ...
  validate(): { ok: boolean; errors: string[] } {
    ...
    for (const t of this.def.transitions || []) {
      ...
      if (this.isTerminal(t.from))
        errors.push(
          `terminal state '${t.from}' has an outgoing transition '${t.on}'`
        );
    }
...
export function loadStateMachines(dir: string = "lifecycle"): Record<string, StateMachine> {
  const out: Record<string, StateMachine> = {};
  const abs = path.isAbsolute(dir) ? dir : path.join(process.cwd(), dir);
  if (!fs.existsSync(abs)) return out;
  for (const file of fs.readdirSync(abs).filter((f: string) => f.endsWith(".json"))) {
