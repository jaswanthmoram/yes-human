...
export function resolveRole(role: any, policy: any = DEFAULT_POLICY, seen: any = new Set()) {
 ...
 * @returns {boolean}
 */
export function hasPermission(role: any, permission: any, policy: any = DEFAULT_POLICY) {
  const deny = policy.deny || [];
  if (deny.some((rule: any) => rule.permission === permission)) return false;
  const permissions = resolveRole(role, policy) as Set<string>;
  return (
    permissions.has('*') ||
    permissions.has(permission) ||
    Array.from(permissions).some((p: string) => p.endsWith(':*') && permission.startsWith(p.slice(0, -1)))
  );
}

export function assertPermission(role: any, permission: any, policy: any = DEFAULT_POLICY) {
