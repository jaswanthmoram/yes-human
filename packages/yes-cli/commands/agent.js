...
export async function cmdAgent(args, repoRoot) {
  ...
  if (sub !== 'create') {
    console.error('Usage:');
    console.error('  yes agent create <category>.<agent-name> --triggers "<t1>,<t2>" --aliases "<a1>" --summary "<summary>"');
  ...
  if (!agentId || agentId.startsWith('--')) {
    console.error('Usage: yes agent create <category>.<agent-name> --triggers "<t1>,<t2>" --aliases "<a1>" --summary "<summary>"');
    return 1;
  }
  ...
  const triggers = triggersVal.split(',').map(t => t.trim()).filter(Boolean);
  const aliases = aliasesVal.split(',').map(a => a.trim()).filter(Boolean);
...
`;
  ...
  if (skipBuild) {
    console.log('Skipping compilation and host bundle rebuild due to --skip-build flag.');
  } else {
    console.log('Running compilation and index rebuild...');
    const compileResult = spawnSync('node', [path.join(repoRoot, 'packages/yes-cli/commands/compile.js')], { cwd: repoRoot, stdio: 'inherit' });
    if (compileResult.status !== 0) {
      console.error('✗ Registry compilation failed.');
      return 1;
    }

    const buildResult = spawnSync('node', [path.join(repoRoot, 'packages/yes-cli/index.js'), 'build', 'all'], { cwd: repoRoot, stdio: 'inherit' });
  ...
  console.log('✓ Agent successfully registered, indexed, and bundles updated!');
  return 0;
}
