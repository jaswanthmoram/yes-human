...
function stagingDossier() {
  ...
  let i = 0;
  while (sources.length < MIN_STAGING_SOURCES)
    sources.push({ ...github, url: 'https://github.com/example/repo-' + i++ });
...
test('self-ref-only dossier fails staging gate', () => {
  ...
  d.sources = Array.from({ length: MIN_STAGING_SOURCES }, (_, i) => ({
    url: 'https://github.com/yes-human/yes-human',
    license: 'MIT',
