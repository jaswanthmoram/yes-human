...
import { connectorManifest, selectConnectorsForTask, validateConnectorCall } from '../../packages/yes-connectors/index.js';

const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..');

test('connector protocol selects profile-enabled connectors for matching tasks', () => {
  const connectors = selectConnectorsForTask('inspect github pull request', { repoRoot, profileName: 'minimal', agentId: 'engineering.code-reviewer' });
...
test('connector validation enforces auth env vars without exposing secret values', () => {
  const connector = { id: 'github', enabled: true, required_auth: true, env_var: 'YES_TEST_MISSING', allowed_agents: ['engineering.code-reviewer'] };
