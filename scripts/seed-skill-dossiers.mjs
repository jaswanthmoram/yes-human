...
function poolFor(domain) {
  return [...(seeds.categories[domain] || []), ...(seeds.categories['meta-system'] || [])].filter((s) => ALLOWED.has(s.license));
}
function pickSeeds(domain, key) {
  ...
  const out = [];
  const add = (arr, n) => { let i = Math.abs(hash(key + arr.length)) % (arr.length || 1); while (arr.length && out.length < 12 && n > 0) { const s = arr[i % arr.length]; if (!out.find((x) => x.url === s.url)) out.push(s); i++; n--; } };
  add(official, 3); add(github, 9);
  if (out.length < 8) add(pool, 12 - out.length);
  return out.slice(0, 12);
}
function buildSources(domain, key, hint) {
  return pickSeeds(domain, key).map((seed, idx) => ({
    url: seed.url, source_type: seed.source_type, license: seed.license,
    ...
    copy_policy: 'patterns_only', last_updated: '2026-05-15T00:00:00Z'
  }));
}
...
for (const fp of walk(path.join(repoRoot, 'content/skills'))) {
  ...
  const dossier = {
    ...
    domain,
    sources,
    scores: { source_count: sources.length, official_docs: scores.official_docs, github_quality: scores.github_quality, license_safety: scores.license_safety, maintenance: scores.maintenance, pattern_clarity: scores.pattern_clarity, testability: scores.testability, total: scores.total },
