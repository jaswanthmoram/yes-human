...
function getRouteCandidates(task, routes, limit = 20) {
  const queryTokens = String(task || '').toLowerCase().split(/[^a-z0-9+#]+/).filter(t => t.length > 2);
  ...
  const filtered = scored.filter(s => s.score > 0).sort((a, b) => b.score - a.score).map(s => s.route);
  ...
  try {
    ...
    if (fs.existsSync(routesPath)) {
      try {
        ...
        console.error(`⚠ [LLM Assist Hook] Failed to parse routes.json: ${err.message}`);
      }
    }
...
`;
    ...
    if (routeId && routeId !== 'NONE' && routeId.startsWith('route.')) {
      console.log(`[Router Hook] LLM mapped task to routing hint: ${routeId}`);
      return {
        allowed: true,
    ...
    console.error(`⚠ pre-route-llm-assist hook error: ${err.message}`);
    return { allowed: true };
  }
}
